...
from videoxlpro.constants import IGNORE_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, IMAGE_TOKEN_INDEX
from videoxlpro.train.llava_trainer import LLaVATrainer

# from transformers import T5Tokenizer, T5EncoderModel
from videoxlpro import conversation as conversation_lib
...
class ModelArguments:
    ...
    rlt_mask_space: str = field(default="pixel", metadata={"help": "WHERE the drop test compares patches (orthogonal to rlt_mask_mode, which is WHAT it compares them against). 'pixel' (default, paper): mean abs change in raw normalized pixels. 'embed': distance between patch embeddings, after the patch-embed conv -- robust to sensor noise/codec grain/lighting flicker that the conv averages out but a pixel diff cannot separate from real change, and it measures the error reuse ACTUALLY incurs (a dropped token reuses an embedding, not pixels). Applies to RLT and to APT-Temporal's dirty-tile check alike. It carries MORE weight under APT-Temporal: the pixel test's sensitivity is proportional to the local spatial gradient (brightness constancy: a flat region cannot express its own motion in intensity differences), and APT merges cells precisely on low intensity dispersion -- so spatial merging systematically routes the temporal check into the regime where it is least able to detect change, and the two methods' drop sets stop being independent. See dirty_subtile_mask_embed's docstring for the derivation and for what 'l2' does and does not fix."})
    rlt_embed_threshold: float = field(default=0.34, metadata={"help": "Drop threshold for rlt_mask_space='embed'. Shared with APT-Temporal, like rlt_threshold. SEPARATE from rlt_threshold on purpose: that one is a pixel-scale value, and an embedding distance is not on that scale -- the two are NOT interchangeable. Default 0.34 is calibrated, not guessed: on 5 MLVU videos x 128 frames it keeps 55.7% of tokens, matching the 56.9% that the default rlt_threshold=0.2 keeps, so switching mask_space swaps the drop RULE while holding the amount of reuse fixed and the comparison stays honest. Same train/eval-must-match reasoning as rlt_threshold applies. Equivalent for rlt_embed_metric='cosine' is ~0.022."})
    apt_temporal_window: int = field(default=1, metadata={"help": "APT-Temporal only: how many frames share ONE entropy partition. 1 (default) = a fresh independent partition per frame, i.e. exactly the previous behaviour. >1 builds one partition per window from the window's MAX entropy, which makes the shape-match condition true by construction inside a window and converts `missed_reuse` (cells that did not change but could not be carried forward because the quadtree boundaries wobbled across the entropy threshold between frames -- pure loss, no information-theoretic justification) into real reuse. It helps COARSE cells most, which is where APT's value is: a scale-s cell previously needed all s^2 base cells to keep the same scale code, so the penalty grew with cell AREA. Cost: the partition is window-optimal rather than frame-optimal, so slightly more tokens per frame, growing with this value. Windows also break at detected cuts, so this is a MAXIMUM length, not a fixed stride. Measure last_stats['missed_reuse'] at window=1 first -- that IS the upper bound on what this can recover."})
    apt_temporal_partition_mode: str = field(default="entropy", metadata={"help": "APT-Temporal only: WHAT the spatial partition is built from. 'entropy' (default, plain APT): each frame's quadtree comes from per-patch Shannon entropy, i.e. spatial flatness -- which competes with RLT for the same redundancy, since flat regions are disproportionately the ones RLT would have dropped anyway (measured on MLVU: coarse cells are temporally quiet 51.6% of the time vs 38.2% for fine cells), and a REDUNDANT cell emits zero tokens at ANY scale, so merging a still region buys nothing and costs fidelity. 'survivor' (RLT-first): the partition is built from what RLT could NOT drop -- a coarse cell exists only where every base tile survived AND their run lengths agree -- so the spatial saving is additive on top of the temporal one by construction. Watch missed_reuse in this mode: a tile that was dirty at t-1 (mergeable) and goes quiet at t cannot be carried from a coarse cell, and that motion-stops transition is not rare."})
    apt_temporal_run_tol: int = field(default=0, metadata={"help": "APT-Temporal, apt_temporal_partition_mode='survivor' only: max allowed spread (max - min) of forward run length inside one merged cell. 0 = exact agreement. Merging a tile that lives 1 frame with one that lives 10 forces the merged token to refresh every frame: the shortest lifetime dominates. What that costs is FIDELITY, not tokens -- the token effect runs the other way, since one merged token refreshing every frame still beats four separate ones (measured, 10 MLVU videos x 128 frames, persist=False: tol=0 -> 278 tokens/frame with missed_reuse 0.042; tol=999 -> 241 with 0.110). The price of a loose tolerance is that long-lived tiles, which could have been carried at full resolution for free, get dragged into a cell re-encoded every frame at reduced resolution, and the churnier partition raises missed_reuse. Requiring agreement also makes the merged token's own run length well defined."})
    apt_temporal_persist: bool = field(default=False, metadata={"help": "APT-Temporal, apt_temporal_partition_mode='survivor' only: keep a coarse cell alive while everything inside it stays QUIET, so a merged token is carried for its whole run instead of fragmenting the moment its tiles stop changing. Without this the merge rule requires all tiles to have CHANGED this frame, so the instant motion stops the cell splits back to base scale, shape_match against the previous frame's coarse cell fails, and every tile inside pays a fresh re-encode -- for a 4-tile block that merges then stays still 5 frames that is 5 tokens, against 4 for never merging and 1 with persistence, i.e. merging something about to go still costs MORE than leaving it alone. Measured on 10 MLVU videos x 128 frames (run_tol=0): off -> 260 tokens/frame, 66.8% dropped, missed_reuse 0.042; on -> 228 tokens/frame, 71.0% dropped, missed_reuse 0.000 (fragmentation was its entire source). It is a TRADE, not a free win: the region stays at merged resolution (4.09x mean vs 3.06x) through the still stretch instead of refreshing at full resolution. Both are still finer than the entropy partition's 6.12x. Default off so the un-persisted result stays reproducible."})
    apt_temporal_cut_threshold: float = field(default=0.8, metadata={"help": "APT-Temporal only, and only consulted when apt_temporal_window > 1: fraction of dirty base tiles above which a frame starts a new partition window. A cut trips essentially every base tile at once, so the dirty mask is already the detector -- no scene-detection pass. A boundary at a cut is FREE: every cell there is dirty hence FRESH anyway, so the boundary's cost and the cut's cost are the same cost paid once. It need not be a semantically correct shot detector -- fast pans and flashes also trip everything, and those are also frames where nothing was reusable. The per-frame dirty fraction is strongly bimodal on edited video, so read this off a histogram of your own footage rather than trusting the default."})
    rlt_embed_metric: str = field(default="l2", metadata={"help": "How rlt_mask_space='embed' compares patch embeddings. 'l2' (default): euclidean distance over the clip's mean patch-embed norm -- dimensionless, so the threshold reads as a fraction of a typical patch's magnitude. 'cosine': direction only (1 - cos_sim); invariant to per-patch gain like exposure drift, but over-keeps flat low-norm regions such as sky, whose direction wanders freely."})
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        input_ids, labels = tuple([instance[key] for instance in instances] for key in ("input_ids", "labels"))
        time_embedding = [instance.get("time_embedding", None) for instance in instances]
        # exit(0) &&&
...
def get_model(model_args, training_args, bnb_model_from_pretrained_args):
    ...
    if model_args.model_class_name is not None:
        ...
        elif "qwen" in model_args.model_name_or_path.lower() or (cfg_pretrained is not None and "qwen" in cfg_pretrained.model_type.lower()):
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.rlt_mask_space = model_args.rlt_mask_space
        model.config.rlt_embed_threshold = model_args.rlt_embed_threshold
        model.config.rlt_embed_metric = model_args.rlt_embed_metric
        model.config.apt_temporal_window = model_args.apt_temporal_window
        model.config.apt_temporal_cut_threshold = model_args.apt_temporal_cut_threshold
        model.config.apt_temporal_partition_mode = model_args.apt_temporal_partition_mode
        model.config.apt_temporal_run_tol = model_args.apt_temporal_run_tol
        model.config.apt_temporal_persist = model_args.apt_temporal_persist
